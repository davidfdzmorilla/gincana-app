...
const agregarUsuario = async (req, res) => {
  const { nombre, email, telefono, password, rol, edad, equipo, foto_perfil } = req.body;
  ...
  try {
    ...
    const [equipos] = await db.query('SELECT * FROM teams WHERE nombre = ?', [nombre_equipo]);
    let assignedEquipoId;
    if (equipos.length > 0) {
      assignedEquipoId = equipos[0].id;
    } else {
      const [equipoResult] = await db.query('INSERT INTO teams (nombre) VALUES (?)', [nombre_equipo]);
    ...
    await db.query('INSERT INTO runners (user_id, edad, equipo_id) VALUES (?, ?, ?)', [
      userId,
      edad || null,
      assignedEquipoId,
    ]);
